-- Eigene Fassung von qbx_core/config/server.lua (Weg C, docs/checkliste.md, Befund 11).
-- resources.txt kopiert diese Datei nach [vendor]/[qbx]/qbx_core/config/server.lua.
-- Grundlage: qbx_core v1.24.0. Nach einem Update von qbx_core die Original-Datei mit dieser vergleichen.
--
-- Abweichungen vom Original:
--   - sendPaycheck: Die Meldung nennt "Grundsicherung" (Job unemployed) bzw. "Gehalt als <Beruf>" und bleibt
--     7 Sekunden stehen. Original: locale('info.received_paycheck'), 3 Sekunden.
-- Ändert sich paycheckTimeout, auch paycheckMinutes in [local]/spielerinfo/config.lua anpassen.
...
return {
    ...
    sendPaycheck = function (player, payment)
        ...
        -- Geändert: Meldung nennt Grundsicherung bzw. Beruf und bleibt 7 Sekunden stehen
        local job = player.PlayerData.job
        local text
        if job.name == 'unemployed' then
            text = ('Grundsicherung erhalten: %s $ aufs Konto'):format(payment)
        else
            text = ('Gehalt als %s erhalten: %s $ aufs Konto'):format(job.label, payment)
        end
        Notify(player.PlayerData.source, text, 'success', 7000)
